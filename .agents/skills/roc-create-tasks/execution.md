# Execution through confirmed merge

Use the CLI entrypoint selected in SKILL.md for every command, including on the
execution host. Replace `npx roc-it@latest` with `bun "$ROC_CLI_ENTRY"` when set.
