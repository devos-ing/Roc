...
export async function loadDefaultSkillPolicy(
  ...
  const raw = await Bun.file(join(agentsRoot, ".skill-lock.json")).json().catch(() => undefined);
...
export function buildDefaultSkillConfig(
  ...
  return skills.map((skill) => {
    ...
    const allowed = skill.name.startsWith("ponytail:")
      || skill.name.startsWith("i-have-adhd:")
      || (input.allowedStandaloneSkillNames.has(skill.name) && skill.path === standalonePath);
