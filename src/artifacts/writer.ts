...
async function assertRealDirectory(path: string): Promise<void> {
  ...
  if (stats.isSymbolicLink()) throw new Error(`Artifact path component is a symbolic link: ${path}`);
  if (!stats.isDirectory()) throw new Error(`Artifact path component is not a directory: ${path}`);
}
...
function assertContained(root: string, candidate: string): void {
  ...
  if (
    ...
    throw new Error(`Artifact directory is outside the project root: ${candidate}`);
  }
}
...
export async function writeTicketArtifact(
  ...
  if (canonicalTicketDirectory !== ticketDirectory) {
    throw new Error(`Artifact ticket directory changed during validation: ${ticketDirectory}`);
  }
  ...
  const temporary = join(canonicalTicketDirectory, `.${task.id}.${randomUUID()}.tmp`);
  ...
  try {
    ...
    if (dirname(canonicalTemporary) !== canonicalTicketDirectory) {
      throw new Error(`Artifact temporary file is outside the ticket directory: ${temporary}`);
    }
    ...
    if (await realpath(ticketDirectory) !== canonicalTicketDirectory) {
      throw new Error(`Artifact ticket directory changed before persistence: ${ticketDirectory}`);
    }
