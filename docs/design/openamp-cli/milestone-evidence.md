...
| 里程碑 | 實作與驗證結果 |
| --- | --- |
| M1 | `openamp`／`--resume` 啟動 Pi 原生 TUI；功能 branch 與 worktree 專用且可恢復；來源 checkout 的 dirty file 保持原狀；正常 agent command boundary 阻止 remote mutation。 |
...
| M4 | Delivery 在 validation 後啟動獨立 read-only Review，綁定 head/base/requirements hash；記錄 publication intent 與 command ledger；lost push/PR response 先查 remote；追問更新同一 PR；無 merge operation。 |
...
核心故障案例在 `test/openamp/openamp.test.mjs`：兩個 writer 的循序整合與衝突保留、第三個 child 排隊、錯誤 session 不接收結果、矛盾 Review JSON 拒絕、Review rejected 零發布，以及 push／PR response 遺失後不重複建立 PR。
