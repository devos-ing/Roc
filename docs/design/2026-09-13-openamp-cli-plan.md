...
日期：2026-09-13。狀態：規劃完成稿，供實作前審閱。這份文件不代表功能已完成，也不啟動實作。
...
以下是本計畫建議的預設，不應混同已實測能力：Node.js 執行、先沿用 Pi 原生 TUI、本機單一使用者、同時最多兩個子 agent、同一功能共用一個主 worktree，以及交付前獨立審查。獨立審查政策仍待使用者回覆。

## 2. 第一版要完成的使用情境

使用者在 Git repository 執行 OpenAmp，登入模型供應商後提出需求。主 agent 可以直接修改，或委派讀取型調查與獨立實作。使用者能看到每個 agent 在做什麼、補充指令、停止工作，並於退出後恢復對話。

例如「加入團隊邀請，請另一個 agent 先查現有登入流程」。主 agent 保持對話，調查 agent 回傳來源證據。需要平行實作時，寫入者取得獨立 worktree，主流程逐一整合結果。整體驗證通過後，OpenAmp 自動 commit、push 功能分支並建立一個 PR，再回報連結與驗證證據。
...
| 修改整合、驗證、獨立審查政策及自動 PR | 自動合併、GitHub merge queue、完整 CI 修復循環 |
...
完成流程為：整合全部選定結果，形成最終本地 commit，執行功能整體驗證，依審查政策檢查該版本，保存發布意圖，再 push 功能分支及建立或更新 PR。驗證後 workspace 必須仍對應該 commit；測試產生的新修改不能暗中夾帶發布。交付使用確定性程式檢查身分與遠端結果，不能只靠「請幫我開 PR」的 prompt。

需要獨立審查時，以另一個 Pi session 檢查固定的最終 head、base 和目前需求版本，回傳接受或具體 findings。審查不能修改實作。修正後重新驗證並審查新的 head，舊結果不能授權新版本發布。
...
| 里程碑 | 交付成果 | 驗收與停止條件 |
| --- | --- | --- |
| M0：技術可行性 | 小型 Pi TUI/extension/RPC 相容性原型及決策紀錄 | Node 正常啟動退出、受控工具載入、兩輪對話、streaming steer、session 恢復、結果持久化可查；失敗先收斂方案，不展開全部模組 |
...
| M3：協同實作 | 子 worktrees、驗證結果 commit、主 workspace 循序整合、獨立審查能力 | 兩項獨立修改整合至同一功能；衝突保留工作；未確認 Git 結果不重播 |
| M4：自動 PR | 最終驗證、審查政策、publication 紀錄、GitHub 建立或更新 PR | 一次需求產生一個 PR；追問更新同一 PR；模擬 lost response 不重複發布；不呼叫 merge |
| M5：替代 Roc | 新包安裝驗證、文件與設定切換、舊執行入口退役 | OpenAmp 完整流程通過；舊 worktree/session 可保留與辨識；發布包無 Roc runtime queue/daemon 路徑 |

M0 原型是未來實作工作的第一步，本次規劃不執行。M1–M3 是內部驗收階段；M4 才具備使用者要求的完整第一版交付行為。
...
規劃基準是目前 checkout 的 HEAD `0469ef7156021e722e710f3db3266a5a372526ad`，加上本次唯讀檢查時存在的未提交修改。HEAD 不是所有目前檔案的完整快照。進入 M0 前重新記錄版本與工作目錄，避免覆蓋同步進行的 ObservationPack 與功能共用 worktree 工作。
...
新產品 runtime 建議 Node.js；既有 Bun 測試工具可暫時沿用。Node 入口必須經過真實 Node 安裝執行測試，不能只靠 Bun 能跑就宣稱相容。若需 build，優先使用 TypeScript 及 Node 支援的輸出，新增套件須說明缺口。
...
設定採明確的一次性移轉，保留原設定備份，不建立永久 fallback 讀取鏈。新執行入口通過完整驗收後，刪除舊 scheduler、queue 入口、專屬 onboarding/runtime skills 及只驗證過時行為的測試，更新 package manifest 和文件。
...
完整垂直案例：開始功能對話 → 調查委派 → 兩項獨立實作 → 途中退出及恢復 → 循序整合 → 整體驗證與審查政策 → 自動建立 PR → 追問更新同一 PR。保留主/子 session、worktree、SHA、有效模型/effort 和驗證紀錄。
...
記錄每次刪減省掉什麼、哪項行為仍成立，以及留下或恢復設計的理由。本次尚未執行這些實驗。
...
推論：沿用 Pi 原生 UI 可以減少首版工作量；專用功能 worktree 可以使自動 PR 的變更範圍更清楚。兩者都須以 M0/M1 的實際使用流程驗證。

實作前只需收斂獨立審查政策，並確認本計畫的建議預設。名稱可留到 M5。正式進入實作時，先固定 M0 工作目錄與基準版本，完成 M0 驗收後才開始 M1。

本次交付為計畫、圖及圖的驗證證據。沒有新增或修改 production code，沒有執行模型付費測試，沒有建立 GitHub PR。

參考：[Roc 架構](../architecture.md)、[Amp CLI](https://ampcode.com/docs/cli)、[Amp agent 委派](https://ampcode.com/docs/orbs/agent-to-agent)、[Amp shipping](https://ampcode.com/docs/orbs/shipping)。共用功能 worktree 的既有需求已摘要在第 6 節；原始的另一項工作計畫不屬於此分支。Pi SDK 與 extensions 的固定版本文件，安裝後位於 `node_modules/@earendil-works/pi-coding-agent/docs/sdk.md` 與 `docs/extensions.md`。分支交接與證據範圍見 [OpenAmp 計畫交接](openamp-cli/README.md)。
