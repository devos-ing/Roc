...
先在同一台電腦演練：A、B 使用獨立 clone 和資料庫，只有 B 跑 daemon。首次用 Pi 的 Codex provider 模型；Claude／GLM 及實體兩機驗證另行記錄。

```mermaid
flowchart LR
    subgraph host["第一階段：同一台電腦"]
      A["A：規劃與發佈<br/>獨立 clone、空的執行佇列"]
      B["B：Roc daemon + Pi RPC<br/>獨立 clone、SQLite、task checkout"]
    end
...
flowchart LR
    ...
    T5 --> smoke["先：單機 A／B + Pi Codex 模型 + GitHub"]
    smoke --> live["後：Pi Claude／GLM、兩機演練"]
...
| remote-operations | REMOTE-05 | A／B 安裝、單一 daemon、隔離與常駐服務文件 |
...
```mermaid
flowchart LR
    D["同一個 Roc daemon"] --> H["PiHarness：每個角色獨立 session"]
    H --> P["Pi RPC：工具與 agent loop"]
    P --> O["openai-codex：Codex 模型"]
    P --> A["anthropic：Claude 模型"]
    P --> Z["zai：GLM 模型"]
```

每次 daemon 啟動固定一組 provider/model；圖中的三條線是可選設定，
不表示同時呼叫三家。Pi 不啟動 Codex CLI 或 Claude Code CLI。
每個角色使用新的 Pi process/session，daemon、資料庫與 task checkout 則依序重用。
Pi 沒有內建 sandbox；`ROC_ALLOW_UNSANDBOXED=1` 是明確確認，
無人看管的部署須另加 OS/container 隔離。
