...
Status: implemented for OpenAmp's native Pi sessions.
...
| --- | --- |
| `op-default-off` | A new interactive OpenAmp change presents a checkbox list with ObservationPack clear. `--plugins` reopens the list for a resumed change. The durable change record retains a confirmed selection; cancellation changes nothing. |
