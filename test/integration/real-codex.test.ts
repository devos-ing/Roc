...
async function waitForDone(
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const status = db.query<{ status: string }, []>("SELECT status FROM tasks WHERE id = 'T1'").get()?.status;
      if (status === "done") return;
      const attempts = db.query<{ role: string; status: string }, []>(
...
async function stopRuntime(running: Promise<number>): Promise<number> {
  ...
  return Promise.race([
    running,
    Bun.sleep(15_000).then(() => { throw new Error("Timed out stopping the real Codex runtime"); }),
  ]);
}

realTest("completes one ticket through Scout, Implement, and detached Review", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "answer.ts"), "export function answer(): number { return 0; }\n");
    await writeFile(join(root, "answer.test.ts"), [
      "import { expect, test } from \"bun:test\";",
    ...
    running = runCli([
      "scheduler", "run", "--backend", "codex", "--repo", root, "--base", baseCommit, "--db", databasePath,
    ], { out: () => {}, err: (text) => errors.push(text) });
    void running.then((code) => { runtimeSettled = { code }; });
    await waitForDone(root, baseCommit, databasePath, () => runtimeSettled, errors);
    ...
    try {
      ...
      acceptedReviews = inspectedDb.query<{ count: number }, []>(
    ...
    expect(task?.attempts.map((attempt) => [attempt.role, attempt.status])).toEqual([
      ["scout", "succeeded"],
      ["implement", "succeeded"],
      ["review", "succeeded"],
    ]);
    expect(task?.attempts.map((attempt) => attempt.modelProfile)).toEqual(["luna", "terra", "sol"]);
    expect(task?.attempts.every((attempt) => attempt.effort === "high" || attempt.effort === "xhigh")).toBeTrue();
    expect(new Set(task?.attempts.map((attempt) => attempt.threadId)).size).toBe(3);
    expect(task?.attempts.every((attempt) => Boolean(attempt.threadId) && Boolean(attempt.turnId))).toBeTrue();
    expect(acceptedReviews).toBe(1);
    expect((task?.actual.inputTokens ?? 0) + (task?.actual.outputTokens ?? 0)).toBeGreaterThan(0);

    const catalogClient = await CodexClient.start();
    try {
      const catalog = ModelListResponseSchema.parse(await catalogClient.request("model/list", {
        limit: 100,
        includeHidden: false,
      })).data.map((model) => model.id);
      expect(task?.attempts.every((attempt) => catalog.includes(attempt.model))).toBeTrue();
    } finally {
    ...
    const lifecycle = log.trim().split("\n").map((line) => JSON.parse(line) as { code: string; runId?: string })
      .filter((record) => record.code === "SCHEDULER_RUN_STARTED" || record.code === "SCHEDULER_RUN_STOPPED");
    expect(lifecycle.map((record) => record.code)).toEqual(["SCHEDULER_RUN_STARTED", "SCHEDULER_RUN_STOPPED"]);
